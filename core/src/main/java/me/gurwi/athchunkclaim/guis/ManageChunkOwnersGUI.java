...
public class ManageChunkOwnersGUI {

    public static HashMap<UUID, PlayerChunk> addOwnerStatus = new HashMap<>();
    public static List<UUID> noBackButtonOwnersGUI = new ArrayList<>();

    public static void open(Player player, PlayerChunk playerChunk) {

        PaginatedGui manageChunkGUI = Gui.paginated()
                .title(Component.text("§8§l» §b§lGESTISCI OWNER CHUNK"))
                .rows(5)
                .create();

        manageChunkGUI.getFiller().fillBorder(ItemBuilder.from(GUIItemsManager.bottomTopFiller()).asGuiItem());

        ChunksDatabase.getChunkOwners(playerChunk).forEach(offlinePlayer -> {
            GuiItem playerHead = ItemBuilder.from(GUIItemsManager.getPlayerHead(offlinePlayer)).asGuiItem(event -> {
                ChunkOwnersRemoveConfirmGUI.open(player, offlinePlayer, playerChunk, false);
            });
        ...
        GuiItem addMember = ItemBuilder.from(Heads.BLACK_PLUS.getItemStack("§a§l+ §7§lAGGIUNGI CO-OWNER")).asGuiItem(event -> {

            if (addOwnerStatus.containsKey(player.getUniqueId())) {
                player.sendMessage("§8§l» §cStai già addando un owner a un chunk!");
                return;
            }

            player.closeInventory();
            addOwnerStatus.put(player.getUniqueId(), playerChunk);
    ...
    public static void open(Player player, PlayerChunk playerChunk, Boolean manageCommand) {
        ...
        ChunksDatabase.getChunkOwners(playerChunk).forEach(offlinePlayer -> {
            GuiItem playerHead = ItemBuilder.from(GUIItemsManager.getPlayerHead(offlinePlayer)).asGuiItem(event -> {
                ChunkOwnersRemoveConfirmGUI.open(player, offlinePlayer, playerChunk, manageCommand);
            });
            manageChunkGUI.addItem(playerHead);
        });
        ...
        GuiItem addMember = ItemBuilder.from(Heads.BLACK_PLUS.getItemStack("§a§l+ §7§lAGGIUNGI CO-OWNER")).asGuiItem(event -> {
            ...
            player.closeInventory();
            addOwnerStatus.put(player.getUniqueId(), playerChunk);
            noBackButtonOwnersGUI.add(player.getUniqueId());
            player.sendTitle("§b§lChi vuoi aggiungere?", "§f§lScrivi in chat il suo nick", 0, 9999999 * 20, 0);
            player.sendMessage("§8§l» §7Scrivi §bAnnulla §7per annullare quest' azione.");
